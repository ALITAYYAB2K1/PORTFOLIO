import React, { useState, useEffect } from "react";
import axios from "axios";
import { Loader } from "lucide-react"; // For loading state

function Timeline() {
  const [timeline, setTimeline] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    const getMyTimeline = async () => {
      try {
        setLoading(true);
        const { data } = await axios.get(
          "http://localhost:8000/api/v1/timeline/getall",
          {
            withCredentials: true,
          }
        );

        // Check if API returned data successfully
        if (data && data.data) {
          // Sort timeline in descending order (newest first)
          const sortedTimeline = [...data.data].sort((a, b) => {
            return parseInt(b.timeline.from) - parseInt(a.timeline.from);
          });
          setTimeline(sortedTimeline);
        } else {
          setError("No timeline data available");
        }
      } catch (error) {
        console.error("Error fetching timeline:", error);
        setError("Failed to load timeline data");
      } finally {
        setLoading(false);
      }
    };

    getMyTimeline();
  }, []);

  if (loading) {
    return (
      <div className="flex justify-center items-center h-64">
        <Loader className="animate-spin h-8 w-8 text-blue-500" />
        <span className="ml-2">Loading timeline...</span>
      </div>
    );
  }

  if (error) {
    return (
      <div className="text-center p-4 bg-red-50 rounded-lg mt-4">
        <p className="text-red-500">{error}</p>
      </div>
    );
  }

  return (
    <div>
      <h1 className="overflow-x-hidden text-[2rem] sm:text-[1.75rem] md:text-[2.2rem] lg:text-[2.8rem] mb-4 font-extrabold">
        Timeline
      </h1>

      {timeline.length === 0 ? (
        <p className="text-center text-gray-500 my-8">
          No timeline entries yet.
        </p>
      ) : (
        <ol className="relative border-s border-gray-200 dark:border-gray-700">
          {timeline.map((element) => (
            <li className="mb-10 ms-6" key={element._id}>
              <span className="absolute flex items-center justify-center w-6 h-6 bg-blue-100 rounded-full -start-3 ring-8 ring-white dark:ring-gray-900 dark:bg-blue-900">
                <svg
                  className="w-2.5 h-2.5 text-blue-800 dark:text-blue-300"
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
                  <path d="M20 4a2 2 0 0 0-2-2h-2V1a1 1 0 0 0-2 0v1h-3V1a1 1 0 0 0-2 0v1H6V1a1 1 0 0 0-2 0v1H2a2 2 0 0 0-2 2v2h20V4ZM0 18a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8H0v10Zm5-8h10a1 1 0 0 1 0 2H5a1 1 0 0 1 0-2Z" />
                </svg>
              </span>
              <h3 className="mb-1 text-lg font-semibold text-gray-900 dark:text-white">
                {element.title}
              </h3>
              <time className="block mb-2 text-sm font-normal leading-none text-gray-400 dark:text-gray-500">
                {element.timeline.from} - {element.timeline.to || "Present"}
              </time>
              <p className="text-base font-normal text-gray-500 dark:text-gray-400">
                {element.description}
              </p>
            </li>
          ))}
        </ol>
      )}
    </div>
  );
}

export default Timeline;
